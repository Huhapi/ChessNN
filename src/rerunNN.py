...
# Load and continue analysis on a neural network.
...
import chessMovePredictor as m

gamemoves,gamepositions = readpgn.get_data()

tensors = []
moves = []
# For all of the games we are analyzing, 
...
model = m.ChessMovePredictor(vocab_size=len(uci_to_index)).to(device)  
model.load_state_dict(torch.load("src/results/model_weights.pth"))

optimizer = torch.optim.Adam(model.parameters(), lr=1e-3)
loss_fn = nn.CrossEntropyLoss()

for epoch in range(16):  # epoch is the number of iterations we use to train our data.
    total_loss = 0
